...
| order_refunds | Backend 1 | 환불 정책은 확정(§5), 컬럼 타입 일부는 TBD |
...
**확정 (실제 적용된 `database/migrations/b1_002_create_order_items.sql` 기준, Sales Step 9 구현 과정에서 반영):**

| Column | Type | Nullable | Default | Constraint | 설명 |
| --- | --- | --- | --- | --- | --- |
| id | UUID | NO | `gen_random_uuid()` | PK | |
| order_id | UUID | NO | - | FK → orders.id | 소속 주문 |
| menu_id | UUID | NO | - | - | 원본 메뉴 ID(참조용, 주문 당시 정보는 아래 컬럼이 기준) |
| menu_name | TEXT | NO | - | - | 주문 당시 메뉴명 스냅샷 |
| unit_price | INTEGER | NO | - | CHECK (unit_price >= 0) | 주문 당시 단가 스냅샷 (컬럼명과 API 응답 필드명이 동일하게 `unitPrice`/`unit_price`다 — 과거 이 문서의 `price`는 실제 컬럼명과 다른 오기였다) |
| quantity | INTEGER | NO | - | CHECK (quantity > 0) | 수량 |

인덱스: `idx_order_items_order_id`(`order_id`)가 이미 적용되어 있다.
...
| Column | Type | Nullable | Default | Constraint | 설명 |
| --- | --- | --- | --- | --- | --- |
| id | BOOLEAN | NO | `true` | PK, **CHECK (id)** | 싱글턴 강제용 고정 컬럼(항상 `true`), API에는 노출하지 않음 |
| bank_name | TEXT | NO | - | - | |
| account_number | VARCHAR | NO | - | - | 문자열로 저장(앞자리 0 보존, 숫자형 변환 금지) |
| account_holder | TEXT | NO | - | - | |
| updated_at | TIMESTAMPTZ | NO | `now()` | - | |

**싱글턴 구조 (확정, 2026-09-19):** `id BOOLEAN PRIMARY KEY DEFAULT true CHECK (id)`로 행이 최대 1개만 존재하도록 DB 레벨에서 강제한다(`id`가 `true`가 아닌 값이면 CHECK 위반, PK 중복이면 두 번째 INSERT가 거부됨). `orders.id` 등 다른 테이블처럼 UUID 참조 ID를 두지 않는다 — `001_백엔드_공통.md` §10의 UUID id 목록에도 `payment_settings`는 포함되지 않으며, 이 테이블은 참조되는 개별 행 식별자가 필요 없는 순수 싱글턴 설정값이기 때문이다. `PATCH /admin/settings/payment`는 `INSERT ... ON CONFLICT (id) DO UPDATE`로 구현해 최초 등록과 갱신을 하나의 upsert로 처리한다. `.env` 초기값(`PAYMENT_SETTINGS_SEED_*`, `npm run seed:payment-settings`)은 최초 1회만 사용하며 이미 행이 있으면 덮어쓰지 않는다.

**계좌 미설정 상태 (확정, 2026-09-19):** 행이 아예 없는 상태(최초 seed도, 관리자 등록도 하지 않은 상태)를 정상적인 초기 상태로 허용한다. 이 경우 `GET /settings/payment`, `GET /admin/settings/payment`는 빈 값이나 임의 값으로 `200`을 반환하지 않고 `404 NOT_FOUND`로 응답한다(`03_API명세서.md` §2-1, §3-3 참고). 배포 절차에서 `npm run seed:payment-settings` 실행이 필수인 이유가 여기에 있다 — `11_배포_체크리스트.md` §0 참고.

---

# 9-1. payment_settings_history

관리자가 `PATCH /admin/settings/payment`로 계좌 정보를 **실제로** 바꿀 때마다 변경 전/후 상태를 남기는 감사(audit) 로그다. `menu_history`(§8-1)와 같은 목적과 구조를 갖되, `payment_settings`가 참조 가능한 개별 행 id를 갖지 않는 싱글턴이라 `menu_id` 같은 대상 FK는 두지 않는다.

| Column | Type | Nullable | Default | Constraint | 설명 |
| --- | --- | --- | --- | --- | --- |
| id | UUID | NO | `gen_random_uuid()` | PK | |
| admin_id | UUID | NO | - | FK → `admins(id)` | 어떤 관리자가 바꿨는지(`AdminGuard`가 설정한 `request.admin.adminId`) |
| old_bank_name | TEXT | NO | - | - | 변경 전 은행명 |
| new_bank_name | TEXT | NO | - | - | 변경 후 은행명 |
| old_account_number | VARCHAR | NO | - | - | 변경 전 계좌번호(문자열, 앞자리 0 보존) |
| new_account_number | VARCHAR | NO | - | - | 변경 후 계좌번호 |
| old_account_holder | TEXT | NO | - | - | 변경 전 예금주 |
| new_account_holder | TEXT | NO | - | - | 변경 후 예금주 |
| changed_at | TIMESTAMPTZ | NO | `now()` | - | 변경이 커밋된 시각 |

**기록 조건 (확정, 2026-09-19):**

- `bankName`/`accountNumber`/`accountHolder` 중 하나라도 실제로 달라진 경우에만 1건 기록한다. 세 필드를 동시에 바꿔도 필드별로 쪼개지 않고 그 PATCH 요청에 대해 1건만 남긴다(같은 요청의 변경 전/후 전체 상태가 한 행에 담김 — `menu_history`와 동일한 원칙).
- 같은 값으로 다시 PATCH하는 no-op 요청은 기록하지 않는다.
- **행이 아예 없는 상태에서의 최초 등록은 "변경"으로 보지 않아 기록하지 않는다.** 이 테이블의 `old_*` 컬럼이 모두 NOT NULL이라 "이전 값 없음" 자체를 표현할 수 없고, 무(無)에서 값을 만드는 것은 있던 값을 바꾸는 것과 성격이 다르다고 보기 때문이다. `.env` 기반 최초 seed(`npm run seed:payment-settings`)도 같은 이유로 이 테이블에 기록하지 않는다(애초에 seed 스크립트는 이 테이블에 접근하지 않는다).

**Transaction (확정, 2026-09-19):** `payment_settings` upsert와 `payment_settings_history` INSERT는 하나의 Transaction(`DatabaseService.withTransaction`)으로 처리된다.

```text
BEGIN
  SELECT ... FROM payment_settings WHERE id = true FOR UPDATE   (변경 전 값 확보 + 잠금, 행이 없으면 null)
  INSERT ... ON CONFLICT (id) DO UPDATE ...                      (payment_settings 값 upsert)
  (이전 행이 있었고 실제로 값이 달라진 경우에만) INSERT INTO payment_settings_history ...
COMMIT / ROLLBACK
```

`payment_settings_history` INSERT가 실패하면(예: `admin_id`가 `admins`에 없어 FK 위반) 방금 실행한 `payment_settings` upsert도 함께 rollback된다 — `menu_history`와 동일한 원자성 보장이다.

**동시성 한계 (알려진 한계, 2026-09-19):** `FOR UPDATE`는 행이 **이미 존재할 때만** 잠금 효과가 있다. 두 관리자가 완전히 같은 순간에 "행이 없는" 최초 등록을 동시에 시도하는 경우(둘 다 `FOR UPDATE`에서 0행을 봄)에는 이 잠금으로 순서를 보장하지 못한다 — 다만 이 경우도 `payment_settings` 자체는 PK/CHECK 제약으로 데이터 손상 없이 "마지막 커밋 승리"로 정상 처리되며, 유실되는 것은 극히 드문 이 경합에서의 이력 기록 1건뿐이다(운영 규모상 실질적 영향은 낮다고 판단해 별도 advisory lock 등을 추가하지 않았다).

**조회 API 없음:** `menu_history`와 동일하게 이번 단계에서는 이 이력을 읽는 관리자 API/UI를 만들지 않는다. 현재는 DB 내부 감사/추적 기록 용도다.

FK에 `ON DELETE`를 지정하지 않았다(기본 `NO ACTION`) — `admins`에 삭제 API가 없어 현재 단계에서는 삭제 시 정책을 고민할 필요가 없다.
...
erDiagram
    ...
    ADMINS ||--o{ PAYMENT_SETTINGS_HISTORY : "changes (admin_id)"
    ...
    PAYMENT_SETTINGS_HISTORY {
        uuid id PK
        uuid admin_id FK
        varchar old_bank_name
        varchar new_bank_name
        varchar old_account_number
        varchar new_account_number
        varchar old_account_holder
        varchar new_account_holder
        timestamptz changed_at
    }
...
`menus`와 `orders`/`order_items` 사이에는 FK를 걸지 않는다. `order_items.menu_id`는 참조용일 뿐이며, 실제 주문 내용의 기준은 스냅샷 컬럼(`menu_name`, `unit_price`)이다 — 메뉴가 삭제/변경되어도 과거 주문에는 영향이 없어야 하기 때문이다(다만 메뉴 삭제 기능 자체가 MVP에 없다).
...
(`b2_003`/`b2_004`는 원래 payment_settings용으로 예상했던 번호였으나, menus `updated_at` 컬럼 추가와 `menu_history` 테이블 생성 migration이 먼저 그 번호들을 사용했다. payment_settings는 다음 번호(`b2_005`)를 사용한다 — 이미 적용된 migration은 수정하지 않으므로 번호를 되돌리지 않는다. `payment_settings_history`는 그다음 번호인 `b2_006`을 사용한다.)
